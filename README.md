...
## Removing GitHub Collaborators (script)

If you need to remove other collaborators from this repository so it's only you, a helper script is provided at `scripts/remove_collaborators.sh`.

Usage:

1. Ensure you have `curl` installed. `jq` is optional but recommended.
2. Run the script and follow prompts:

```bash
chmod +x scripts/remove_collaborators.sh
./scripts/remove_collaborators.sh
```

The script will prompt for repository owner, repo name (defaults to `HealthCare-Plus`), and a GitHub personal access token (or read `GITHUB_TOKEN` from the environment). It lists current collaborators and asks for confirmation before removing any users.

Important: You must be an admin of the repository and the token must have appropriate scopes (`repo` or `public_repo`).
